...
def filter_by_year(statistics, year, yearid):
    ...
    """
    filtered=[]
    filtered=[x for x in statistics if x[yearid] == str(year)]
...
#    print(statistics)
    ...
    top_players=[(p[info["playerid"]],formula(info,p)) for p in statistics]
                     
    
    top_players.sort(key= lambda x: x[1],reverse=True)
    
#    print(top_players[:numplayers])
    return top_players[:numplayers]
...
def lookup_player_names(info, top_ids_and_stats):
    ...
    """
    table=read_csv_as_nested_dict(info["masterfile"], info["playerid"],
                                  info['separator'], info['quote'])
    lookup=[]
    for iddd in top_ids_and_stats:
        str1=str(format(iddd[1],'.3f'))
        str2=str(table[iddd[0]][info['firstname']])+' '+ str(table[iddd[0]][info['lastname']])
        str3= str1+ ' --- ' + str2
        lookup.append(str3)
...
def compute_top_stats_year(info, formula, numplayers, year):
    ...
    """
    batting=read_csv_as_list_dict(info['battingfile'],',','"')
    
    filtered_batting= filter_by_year(batting,year,info['yearid'])
    top_players=top_player_ids(info,filtered_batting,formula,numplayers)
    lookup_players=lookup_player_names(info, top_players)
    
    return lookup_players
...
#            print('the if else condition')
            agg[player]={f:0 for f in fields}
            agg[player][playerid]=player
            for field in fields:
    #            print(field)
                agg[player][field]=agg[player][field]+int(row[field])
        else:
...
def compute_top_stats_career(info, formula, numplayers):
    ...
    agg=aggregate_by_player_id(batting,info['playerid'], info['battingfields'])
#    print(agg.keys())
    for player in agg:
        newlist.append(agg[player])
    topstats=top_player_ids(info, newlist, formula, numplayers)
    lookup=lookup_player_names(info, topstats)
